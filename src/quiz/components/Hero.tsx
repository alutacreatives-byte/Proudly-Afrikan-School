...
import { ArrowRight, Sparkles, Zap, ShieldCheck, Flame, HelpCircle, CheckCircle2, RefreshCw } from 'lucide-react';
import { useAuthCredit } from '../../context/AuthCreditContext';
import { useDynamicInspiration, QUIZ_TOPICS_POOL } from '../../data/inspirationTopics';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-7 space-y-6 flex flex-col justify-start">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/90 border border-stone-300/80 rounded-full shadow-sm text-[10px] sm:text-xs font-mono font-bold tracking-wider uppercase text-stone-800 self-start h-8 whitespace-nowrap">
            <span className="w-2 h-2 rounded-full bg-[#E52E5E] inline-block animate-pulse shrink-0"></span>
            <span className="truncate">PROUDLY AFRIKAN EDUCATION • QUIZ GENERATOR</span>
          </div>
            ...
            <div className="flex items-center justify-between border-b border-stone-200 pb-3 h-9">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-1 text-stone-400 hover:text-[#E52E5E] transition-colors rounded-full hover:bg-stone-200/50 cursor-pointer"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 min-h-[10.5rem]">
              {inspirationTopics.map((item, idx) => (
                <button
                  ...
                  className="h-11 px-3.5 bg-white hover:bg-orange-50/60 border border-stone-200/90 hover:border-orange-300 text-stone-800 hover:text-[#E52E5E] font-medium text-xs sm:text-sm rounded-full transition-all shadow-sm flex items-center gap-2 text-left truncate cursor-pointer"
                >
