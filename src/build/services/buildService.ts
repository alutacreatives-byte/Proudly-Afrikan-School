...
export async function generateMindMap(params: {
  ...
  const data = await postJson<any>('/api/generate-mind-map', params);
  const resource: MindMapResult = {
    id: data.id || `mm-${Date.now()}`,
    title: params.topic,
    subject: params.subject,
    topic: params.topic,
    gradeLevel: params.gradeLevel || 'Senior Secondary / High School (Grades 9-12)',
    rootNode: data.rootNode || (data.data && data.data.rootNode),
    summary: data.summary || (data.data && data.data.summary),
    sourceDocName: params.sourceFileName,
